...
export class PorterServiceAPI {
  ...
  static async createPorter(data: PorterFormData): Promise<Porter> {
    try {
      ...
      console.log(`✅ Successfully created porter ${createdPorter.name} (ID: ${createdPorter.id}) via API`)
  ...
  static async updatePorter(id: number, data: Partial<PorterFormData>): Promise<void> {
    try {
      ...
      if (data.break_duration_minutes !== undefined) updates.break_duration_minutes = data.break_duration_minutes
